...
module.exports = async params => {
  const data = await Promise.all(
    ['transfers', 'gmp'].map(d => new Promise(async resolve => {
      ...
      const { data } = { ...response };
      resolve(toArray(data).map(_d => {
        const { num_txs, volume, fee, users } = { ..._d };
        return { ..._d, [`${d}_num_txs`]: num_txs, [`${d}_volume`]: volume, [`${d}_fee`]: fee, [`${d}_users`]: users };
      }));
    }))
  );
  return {
    data: _.orderBy(Object.entries(_.groupBy(data.flatMap(d => d), 'timestamp')).map(([k, v]) => {
      return {
        timestamp: Number(k),
