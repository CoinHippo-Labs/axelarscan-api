            ...
            .map(d =>
              new Promise(
                async resolve => {
                  ...
                  if (!failed && !(success || confirmation) && Object.entries({ ...d }).filter(([k, v]) => k.startsWith(getChainData('axelarnet')?.prefix_address) && !v?.vote).length > MIN_VOTE_PER_POLL_TO_FAILED) {
                    d.failed = true;
                    _updated = true;
                  }
