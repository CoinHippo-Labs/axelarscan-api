...
const getEVMBalance = async (wallet_address, contract_data, provider, chain) => {
  ...
  if (wallet_address && address && provider) {
    ...
    for (const url of toArray(chain_data?.endpoints?.rpc)) {
      try {
        const rpc = axios.create({ baseURL: url });
        const response = await rpc.post('', { jsonrpc: '2.0', method: address === ZeroAddress ? 'eth_getBalance' : 'eth_call', params: address === ZeroAddress ? [wallet_address, 'latest'] : [{ to: address, data: `${keccak256(toUtf8Bytes('balanceOf(address)')).substring(0, 10)}000000000000000000000000${wallet_address.substring(2)}` }, 'latest'], id: 0 }).catch(error => { return { error: error?.response?.data }; });
        const { data } = { ...response };
        const { result } = { ...data };
        if (result) {
