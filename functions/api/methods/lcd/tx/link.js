...
const { normalizeLink, updateLink, updateSend } = require('../../transfers/utils');
const { getTokensPrice } = require('../../tokens');
const { get, read, write } = require('../../../services/index');
const { TRANSFER_COLLECTION, DEPOSIT_ADDRESS_COLLECTION, getChainsList, getChainKey, getChainData } = require('../../../utils/config');
...
module.exports = async (lcd_response = {}) => {
  ...
  const { attributes } = { ..._.head(toArray(logs).flatMap(l => toArray(l.events).filter(e => equalsIgnoreCase(e.type, 'link')))) };
  ...
  const { sender, chain, recipient_addr, asset } = { ...data };
  let {
    id,
    type,
  ...
  if (sender_address?.startsWith(getChainData('axelarnet').prefix_address) && getChainData(sender_chain)) {
    ...
    const { send, link } = { ..._.head(response?.data) };
  ...
  if (transfer_data) {
    const { send, unwrap, type } = { ...transfer_data };
