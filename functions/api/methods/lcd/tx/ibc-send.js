...
const { TRANSFER_COLLECTION, getAssetData } = require('../../../utils/config');
...
module.exports = async (lcd_response = {}) => {
  ...
  if (height && toArray(logs).length > 0) {
    ...
    const { end_block_events } = { ...response };
    ...
    if (toArray(logs).findIndex(l => toArray(l.events).findIndex(e => equalsIgnoreCase(e.type, 'send_packet')) > -1) < 0) {
      ...
      for (const event of events) {
        ...
        const { sender } = { ...toJson(toArray(attributes).find(a => a.key === 'packet_data')?.value) };
