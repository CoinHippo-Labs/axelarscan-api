      ...
      if (_id) {
        ...
        if (!code && height && destination_chain && packet_data_hex) {
          ...
          if (chain !== 'axelarnet' && lcd) {
            ...
            if (txhash) {
              const { attributes } = { ..._.head(toArray(logs).flatMap(l => toArray(l.events).filter(e => equalsIgnoreCase(e.type, 'write_acknowledgement') && packet_sequence === toArray(e.attributes).find(a => a.key === 'packet_sequence')?.value))) };
