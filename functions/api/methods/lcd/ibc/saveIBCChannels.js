...
module.exports = async (path = '/ibc/core/channel/v1/channels', lcd_response = {}) => {
  ...
  if (channels) {
    ...
    if (lcd) {
      ...
      while (next_key) {
        const response = await lcd.get(path, { params: { 'pagination.key': next_key } }).catch(error => parseRequestError(error));
        const { data } = { ...response };
          ...
          new Promise(
            async resolve => {
              ...
              if (!chain_id || !escrow_address || (counterparty && !counterparty.escrow_address) || moment().diff(moment((0 || 0) * 1000), 'minutes', true) > 240) {
                const response = await lcd.get(`/ibc/core/channel/v1/channels/${channel_id}/ports/${port_id}/client_state`).catch(error => parseRequestError(error));
