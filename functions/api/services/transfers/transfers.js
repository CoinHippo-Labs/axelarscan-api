...
module.exports = async (
            ...
            bool: {
              ...
              should:
                cosmos_chains_data.flatMap(c => {
                  ...
                  return (
                    _.uniq(_.concat(id, Object.keys({ ...overrides })))
                      .map(id => {
                        return {
                          match_phrase: {
    ...
    must.push(
      {
        bool: {
          should: [
            ...
            { match: { 'erc20_transfer.deposit_address': depositAddress } },
          ],
          minimum_should_match: 1,
        },
      }
    ...
    must.push(
      {
        bool: {
          should: [
            ...
            { match: { 'erc20_transfer.sender_address': senderAddress } },
          ],
          minimum_should_match: 1,
        },
      }
  ...
  if (txHash) {
    read_params.sort = _.concat(read_params.sort, [{ 'confirm.created_at.ms': 'desc' }]);
  }

  response = await read(collection, query, read_params);
              ...
              try {
                if (!wrap) {
                  ...
                  if (wrap) {
                    ...
                    if (tx_hash) {
                      const chain_data = evm_chains_data.find(c => c?.id === source_chain);
  ...
  if (Array.isArray(response?.data)) {
    ...
    data =
      data.filter(d => {
        ...
        return (
          txhash &&
          (
            !(source_chain && destination_chain && typeof amount === 'number' && typeof value === 'number' && typeof fee === 'number') ||
            (evm_chains_data.findIndex(c => equals_ignore_case(c?.id, destination_chain)) > -1 && !insufficient_fee && (vote || confirm) && !command?.executed) ||
            (cosmos_chains_data.findIndex(c => equals_ignore_case(c?.id, destination_chain)) > -1 && !['axelarnet'].includes(destination_chain) && !insufficient_fee && (vote || confirm) && !(ibc_send?.failed_txhash || ibc_send?.ack_txhash || ibc_send?.recv_txhash)) ||
            (['axelarnet'].includes(destination_chain) && !insufficient_fee && !axelar_transfer) ||
            !(vote?.transfer_id || confirm?.transfer_id) ||
            (unwrap && !unwrap.tx_hash_unwrap) ||
            (evm_chains_data.findIndex(c => equals_ignore_case(c?.id, source_chain)) > -1 && !insufficient_fee && !vote && (command || ibc_send || axelar_transfer))
          )
        );
      });
    ...
    if (data.length > 0) {
      ...
      await sleep((data.length > 1 ? 4 : 0) * 1000);
      updated = true;
    }
  }
  ...
  if (Array.isArray(response?.data)) {
    ...
    data =
      data.filter(d => {
        ...
        return txhash && source_chain && !total && (command?.executed || ibc_send?.ack_txhash || unwrap);
      });
  ...
  if (Array.isArray(response?.data)) {
    ...
    data =
      data.filter(d => {
        ...
        return ['uluna', 'uusd'].includes(denom) && ms < moment('20220401', 'YYYYMMDD').utc().valueOf() && fee > parseFloat((amount * 0.001).toFixed(6));
      });
  ...
  if (Array.isArray(response?.data)) {
    ...
    data =
      data.filter(d => {
        ...
        return txhash && source_chain && typeof height === 'string';
      });
  ...
  if (Array.isArray(response?.data)) {
    ...
    data =
      data.filter(d => {
        ...
        return txhash && source_chain && ((wrap && type !== 'wrap') || (unwrap && type !== 'unwrap') || (erc20_transfer && type !== 'erc20_transfer'));
      });
  ...
  if (['to_fix_value'].includes(status) && Array.isArray(response?.data)) {
    let {
      data,
    } = { ...response };
    ...
    data =
      data.filter(d => {
        ...
        return txhash && source_chain && typeof price !== 'number';
      });
