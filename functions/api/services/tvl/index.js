...
module.exports = async (
  ...
  for (const asset of assets) {
    ...
    // get tvl from rpc
    const evm_tvl = Object.fromEntries(
      await Promise.all(
        _evm_chains_data
          .map(c =>
            new Promise(
              async (resolve, reject) => {
                ...
                const contract_data = contracts?.find(_c =>
                ...
                if (
                  ...
                  result = {
                    ...
                    url: explorer?.url &&
                      `${explorer.url}${
                        explorer.contract_path?.replace(
                          '{address}',
                          contract_address,
                        )
                      }${
    ...
    // get tvl from lcd
    let cosmos_tvl = Object.fromEntries(
      await Promise.all(
        _cosmos_chains_data
          .map(c =>
            new Promise(
              async (resolve, reject) => {
                ...
                result = {
                  ...
                  url: explorer?.url &&
                    ...
                    source_escrow_addresses?.length > 0 &&
                    is_native && id !== axelarnet.id ?
                      `${explorer.url}${
                        explorer.address_path.replace(
                          '{address}',
                          _.last(source_escrow_addresses),
                        )
                      }` :
                      explorer?.url &&
                      explorer.asset_path &&
                      ibc_denom?.includes('/') ?
                        `${explorer.url}${
                          explorer.asset_path.replace(
                            '{ibc_denom}',
                            _.last(ibc_denom.split('/')),
                          )
                        }` :
                        axelarnet.explorer?.url &&
                        axelarnet.explorer.address_path &&
                        escrow_addresses?.length > 0 ?
                          `${axelarnet.explorer.url}${
                            axelarnet.explorer.address_path.replace(
                              '{address}',
                              _.last(escrow_addresses),
                            )
                          }` :
                          null,
