...
const calculate_vesting = config => {
  ...
  const total = fix_decimals(max_supply * total_unlock_percent / 100, decimals);
  ...
  const unlocked =
    current_time.valueOf() >= vesting_until_time.valueOf() ?
      total :
      current_time.valueOf() <= vesting_start_time.valueOf() ?
        0 :
        fix_decimals(total * current_time.diff(vesting_start_time, `${vesting_period}s`) / vesting_until_time.diff(vesting_start_time, `${vesting_period}s`), decimals);
...
module.exports = async (
  ...
  debug = typeof debug === 'boolean' ? debug : equals_ignore_case(debug, 'true');

  const current_total_supply = await total_supply({ asset });
  const inflation_rewards = typeof current_total_supply === 'number' && current_total_supply > max_supply ? fix_decimals(current_total_supply - max_supply, decimals) : 0;
  const initial_unlocked = fix_decimals(max_supply * initial_unlocked_percent / 100, decimals);
