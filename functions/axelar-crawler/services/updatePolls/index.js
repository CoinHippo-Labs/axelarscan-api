    ...
    while (!context || context.getRemainingTimeInMillis() > 300 * 1000) {
      ...
      for (const height of heights) {
        ...
        while (next_key) {
          ...
          const response = await api.post('/', { method: 'lcd', path: '/cosmos/tx/v1beta1/txs', index: true, index_transfer: true, index_poll: true, events: `tx.height=${height}`, 'pagination.key': page_key }).catch(error => parseRequestError(error));
