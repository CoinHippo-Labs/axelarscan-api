...
module.exports = context => {
  ...
  if (api) {
    ...
    const indexTransaction = txhash => api.get('/', { params: { index: true, method: 'lcd', path: `/cosmos/tx/v1beta1/txs/${txhash}` } }).catch(error => parseRequestError(error));
    const index = async i => {
      for (let height = start_block; height < end_block; height++) {
        if (height % num_processes === i) {
          ...
          let next_key = true;
          while (next_key) {
            const response = await api.get('/', { params: { method: 'lcd', path: '/cosmos/tx/v1beta1/txs', events: `tx.height=${height}`, 'pagination.key': typeof next_key === 'string' && next_key ? next_key : undefined } }).catch(error => parseRequestError(error));
            const { data } = { ...response };
